...
const CalendarStep = ({ selectedDate, setSelectedDate, selectedSlot, setSelectedSlot, availableSlots, availableDates, excludedWeekdays, onContinue, bookedSlots }: CalendarStepProps) => {
  ...
  const isSlotInPast = (slot: string): boolean => {
    ...
    // Only Apply this chech for the current day
    if (selectedDay.getTime() === today.getTime()) {
      ...
      const bufferedCurrentTime = new Date(now.getTime() + 5 * 60 * 1000); // Add 5 minutes buffer
  ...
  const isDateDisabled = (date: Date) : boolean => {
    ...
    //Check if the weekday is excluded
    const jsWeekday = date.getDay(); // Sunday - 0, Monday - 1, ..., Saturday - 6
