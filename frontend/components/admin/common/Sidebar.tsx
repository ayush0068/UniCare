...
interface SidebarProps { pendingDoctors?: number }

export default function AdminSidebar({ pendingDoctors = 0 }: SidebarProps) {
    ...
    >
      ...
      <div className="flex items-center h-16 px-4 border-b border-slate-100 flex-shrink-0">
        ...
        <button
          ...
          className={cn(
            'ml-auto flex-shrink-0 flex items-center justify-center w-7 h-7 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors',
            sidebarCollapsed && 'mx-auto'
          )}
        >
          <i className={cn('bi text-sm transition-transform duration-300', sidebarCollapsed ? 'bi-chevron-right' : 'bi-chevron-left')} />
        </button>
      </div>

      {/* Admin Info */}
      <div className={cn('flex items-center gap-3 px-3 py-3 border-b border-slate-100 flex-shrink-0', sidebarCollapsed && 'justify-center px-0')}>
          ...
          return (
            <Link key={item.href} href={item.href} title={sidebarCollapsed ? item.label : undefined}>
              <motion.div
                ...
                className={cn(
                  'relative flex items-center rounded-xl px-3 py-2.5 text-[13px] font-medium transition-all duration-150 group cursor-pointer',
              ...
              >
                <div className={cn('flex items-center justify-center w-5 h-5 flex-shrink-0', !sidebarCollapsed && 'mr-3')}>
      ...
      {/* Bottom */}
      <div className={cn('border-t border-slate-100 p-2 flex-shrink-0', sidebarCollapsed && 'flex justify-center')}>
        <button
          onClick={() => { clearAuth(); window.location.href = '/admin-login'; }}
          title={sidebarCollapsed ? 'Sign out' : undefined}
          className={cn(
            'flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-[13px] font-medium text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors group',
            sidebarCollapsed && 'justify-center w-10 h-10 px-0'
          )}
        >
