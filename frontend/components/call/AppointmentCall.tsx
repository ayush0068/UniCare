...
const AppointmentCall = ({appointment, currentUser, onCallEnd, joinConsultation}: AppointmentCallInterface) => {
    ...
    const memorizedJoinConsultation = useCallback(async (appointmentId: string) => {
            ...
            zp.joinRoom({
                ...
                onJoinRoom: () => {
                    if(isComponentMountedRef.current) {
                        console.log(`Joined Room ${appointment.consultationType} : ${appointment.zegoRoomId}`);
                    }
                },
                onLeaveRoom: () => {
                    if(isComponentMountedRef.current) {
                        if(zpRef.current) {
                            try {
                                ...
                                console.warn('Error Turning off Camera and Microphone');
                            }
                        }
                    }
                }, 
    ...
    }, [appointment?._id, appointment.zegoRoomId, appointment.consultationType, currentUser.id, currentUser.name, memorizedJoinConsultation, onCallEnd]);
      ...
      <div className='bg-white border-b p-4 flex items-center justify-between'>
            <div>
                ...
                <p className='text-sm text-gray-600'>
                    {currentUser.role === 'doctor' ? `Patient: ${appointment.patientId.name}` : `Doctor: ${appointment.doctorId.name}`}
                </p>
            </div>
      </div>
