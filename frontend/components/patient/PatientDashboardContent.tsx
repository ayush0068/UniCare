            ...
            <div className='flex-1 min-w-0'>
              <div className='flex items-start justify-between gap-2 mb-1.5'>
                <div className='min-w-0'>
                  <h3 className='font-bold text-slate-900 text-base leading-tight truncate'>{appointment.doctorId?.name}</h3>
    ...
    <>
      ...
      <div className='uc-font min-h-screen bg-[#F8F7F4] pt-16'>
        <div className='max-w-5xl mx-auto px-4 sm:px-6 py-8 page-animate'>
          ...
          {/* ─── Tabs ────────────────────────────── */}
          <div className='flex gap-1.5 mb-8 bg-white rounded-2xl p-1.5 border border-slate-100 shadow-sm w-full sm:w-fit overflow-x-auto scrollbar-none'>
            {TABS.map(({ id, label, icon: Icon, count }) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={id === 'reports' ? 'hidden xs:inline sm:inline' : ''}>{label}</span>
                  {id === 'reports' && <span className='xs:hidden sm:hidden'>Records</span>}
                  {count !== null && (
