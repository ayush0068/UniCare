...
export const useAppointmentStore = create<AppointmentState>((set, get) => ({
    ...
    joinConsultation: async (appointmentId) => {
        ...
        try {
            ...
            set(state => ({
                appointments: state.appointments.map(apt => apt._id === appointmentId ? {...apt, status: 'In Progress' as const} : apt),
                currentAppointment: state.currentAppointment?._id === appointmentId ? {...state.currentAppointment, status: 'In Progress' as const} : state.currentAppointment,
    ...
    updateAppointmentStatus: async (appointmentId, status) => {
        ...
        try {
            ...
            set(state => ({
                appointments: state.appointments.map(apt => apt._id === appointmentId ? {...apt, status: status as any} : apt),
                currentAppointment: state.currentAppointment?._id === appointmentId ? {...state.currentAppointment, status: status as any} : state.currentAppointment,
